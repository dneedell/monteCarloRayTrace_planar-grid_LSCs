...
%
%   FILE DESCRIPTION:
%       This function looks at the y velocity change for a photon at a
...
function [newvel] = refractY(input,polarangle,tempvel)
    ...
    if rand() < reflectprob
        ...
        %   Send the photon in the other y-direction:
        newvel(2) = -tempvel(2);
    
    %   Else the photon refracts:
    else
        
        %   Define the new velocity as the y-velocity from the input
        %   vector:
        newvel(2) = sign(tempvel(2))*input(4);
        ...
        if tempvel(3) ~= 0
            ...
            %   angle:
            newvel(1) = abs(sin(phi)*sin(theta2))*sign(tempvel(1));
